...
class res_currency(models.Model):
    ...
    def amount_to_text(self, amount, lang):
        amount = '%.2f' % amount
        units_name = ' ' + self.currency_name + ' '
        lis = str(amount).split('.')
        start_word = english_amount(int(lis[0]), lang)
        end_word = english_amount(int(lis[1]), lang)
    ...
    def _convert_nnn(val, lang):
        ...
        if rem == 1:
            ...
            if mod > 0:
                word = word + ' '
        if mod > 0:
            word = word + _convert_nn(mod, lang)
        return word
    
    def english_amount(val, lang):
        denom = dic['denom_id']
        if lang == 'en':
            denom = dic['denom']
        if val < 100:
            return _convert_nn(val, lang)
        if val < 1000:
            return _convert_nnn(val, lang)
        for (didx, dval) in ((v - 1, 1000 ** v) for v in range(len(denom))):
            if dval > val:
                mod = 1000 ** didx
                l = val // mod
                r = val - (l * mod)
                ret = _convert_nnn(l, lang) + ' ' + denom[didx]
                if r > 0:
                    ret = ret + ' ' + english_amount(r, lang)
                if lang == 'id':
                    if val < 2000:
