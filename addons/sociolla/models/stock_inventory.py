...
class stock_inventory_line(models.Model):
    ...
    theoretical_qty_all = fields.Float(string=u'QOH all Locations',related='product_id.qty_available',)
    ...
    def action_open_quants(self):
        ...
        action = self.env['ir.model.data'].xmlid_to_object('stock.product_open_quants')
        if not action:
            action = {
                'view_type': 'form',
        ...
        action['domain'] = "[('product_id','in', %s)]" % (self.product_id.ids)
        action['context'] = ctx
    ...
    def _resolve_inventory_line(self, cr, uid, inventory_line, context=None):
        ...
        inventory_line._compute_theoretical_qty()
